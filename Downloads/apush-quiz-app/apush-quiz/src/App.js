...
const App = () => {
  ...
  const learnModes = [
    ...
    { id: 'why-to-term-when', name: 'Why & Date → Term', description: 'Given the significance and date, identify the term', icon: <BookOpen className="w-5 h-5" /> },
  ];
  ...
  const nextQuestion = () => {
    if (currentQuestion < learnData.length - 1) {
  ...
  // Calculate score with keyword-based partial credit
  const calculateScore = () => {
    let totalScore = 0;
    let maxPossibleScore = learnData.length * 100; // 100 points per question

    learnData.forEach((term, index) => {
      const userAnswer = userAnswers[index];
      let questionScore = 0;

      if (userAnswer && learnMode === 'term-to-why-when') {
        const whyAnswer = typeof userAnswer === 'string' ? userAnswer : (userAnswer.why || '');
        const whenAnswer = typeof userAnswer === 'string' ? '' : (userAnswer.when || '');
        
        if (whyAnswer) {
          questionScore += calculateKeywordScore(whyAnswer, term.why, 70); // 70 points for significance
        }
        if (whenAnswer) {
          questionScore += calculateKeywordScore(whenAnswer, term.when, 30); // 30 points for time period
        }
      } else if (userAnswer && learnMode === 'why-to-term-when') {
        const termAnswer = typeof userAnswer === 'string' ? userAnswer : '';
        if (termAnswer) {
          questionScore += calculateKeywordScore(termAnswer, term.term, 100); // 100 points for term identification
        }
      }

      totalScore += Math.min(questionScore, 100); // Cap at 100 points per question
    });
  ...
  const calculateKeywordScore = (userAnswer, correctAnswer, maxPoints) => {
    ...
    // Define important historical keywords that get extra weight
    const historicalKeywords = [
      'government', 'revolution', 'independence', 'colony', 'colonial', 'british', 'american',
      'economic', 'political', 'social', 'religious', 'freedom', 'rights', 'trade', 'war',
      'conflict', 'expansion', 'territory', 'slavery', 'native', 'european', 'settlement',
      'democracy', 'republic', 'constitution', 'federal', 'state', 'power', 'authority',
      'resistance', 'rebellion', 'reform', 'movement', 'culture', 'society', 'labor',
      'industrial', 'agriculture', 'economy', 'finance', 'taxation', 'representation'
    ];
    ...
    correctWords.forEach(correctWord => {
      ...
      if (userWords.includes(correctWord)) {
        const isHistoricalKeyword = historicalKeywords.includes(correctWord);
        score += isHistoricalKeyword ? 8 : 5; // Higher score for historical keywords
        keywordMatches++;
      } else {
        // Check for partial matches (for dates, names, etc.)
    ...
    if (keywordMatches > totalKeywords * 0.7) {
      score += 10; // Bonus for comprehensive understanding
    }

    // Convert to percentage of maxPoints
    const maxPossibleScore = Math.max(50, totalKeywords * 5); // Minimum threshold
    return Math.min((score / maxPossibleScore) * maxPoints, maxPoints);
  };

  if (isLearnActive) {
    const currentTerm = learnData[currentQuestion];
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="text-sm text-gray-500">
                Question {currentQuestion + 1} of {learnData.length}
              </div>
            </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{ width: `${((currentQuestion + 1) / learnData.length) * 100}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="mb-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-4">
                {learnMode === 'term-to-why-when' ? currentTerm.term : 'Identify the Term'}
              </h2>
              
              {learnMode === 'term-to-why-when' ? (
                ...
                <div className="space-y-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p className="text-gray-700 mb-2"><strong>Why was this significant?</strong></p>
                    <p className="text-gray-800">{currentTerm.why}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p className="text-gray-700 mb-2"><strong>When did this occur?</strong></p>
                    <p className="text-gray-800">{currentTerm.when}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      What is this term?
                    ...
                    <input
                      type="text"
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      placeholder="Enter the historical term..."
            ...
            <div className="flex justify-between">
              ...
              >
                <span>{currentQuestion === learnData.length - 1 ? 'Finish Session' : 'Next'}</span>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">Learning Session Complete!</h2>
            <div className="text-6xl font-bold text-blue-600 mb-4">
              {calculateScore()}%
            </div>
            <p className="text-gray-600 mb-2">
              You answered {Object.keys(userAnswers).length} out of {learnData.length} questions
            </p>
            <p className="text-sm text-gray-500 mb-8">
              Score based on keyword matching and historical understanding
            </p>
            <div className="space-y-4">
              <button
                onClick={resetLearn}
