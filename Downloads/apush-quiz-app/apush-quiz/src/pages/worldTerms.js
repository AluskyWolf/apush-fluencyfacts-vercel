...
const normalizeAnswer = (text) => {
  return text.toLowerCase().trim()
    ...
    // AP World History specific normalizations for common terms
    .replace(/\bhan dynasty\b/g, 'han')
    .replace(/\btang dynasty\b/g, 'tang')
    .replace(/\bsong dynasty\b/g, 'song')
    .replace(/\byuan dynasty\b/g, 'yuan')
    .replace(/\bming dynasty\b/g, 'ming')
    .replace(/\bqing dynasty\b/g, 'qing')
    .replace(/\broman empire\b/g, 'roman')
    .replace(/\bbyzantine empire\b/g, 'byzantine')
    .replace(/\bottoman empire\b/g, 'ottoman')
    .replace(/\bmughal empire\b/g, 'mughal')
    .replace(/\bsafavid empire\b/g, 'safavid')
    .replace(/\binca empire\b/g, 'inca')
    .replace(/\baztec empire\b/g, 'aztec')
...
const calculateSimilarityScore = (userAnswer, correctAnswer, keywords = [], field = null) => {
  ...
  if (field === 'when') {
    ...
    if (userHasDate && correctHasDate) {
      const userDates = userOriginal.toLowerCase().match(/\d+|bce|ce|present|today|century/g) || [];
      const correctDates = String(correctAnswer).toLowerCase().match(/\d+|bce|ce|present|today|century/g) || [];
      
      const normalizeDate = (date) => date.replace(/bce?/g, 'bce').replace(/ad/g, 'ce').replace(/today/g, 'present');
  ...
  // Specific adjustments for World history content
  if (field === 'who') {
    ...
    const specificGroups = ['caliphate', 'sultanate', 'dynasty', 'empire', 'kingdom', 'khanate', 'tsardom', 'shahdom', 'sultanate', 'caliph'];
    const userHasSpecificGroup = specificGroups.some(group => 
      userNormalized.includes(group)
    );
    const correctHasGeneral = /ruler|leader|figure|emperor|king|queen/i.test(String(correctAnswer));
  ...
  if (field === 'where') {
    const locationWords = ['asia', 'africa', 'europe', 'americas', 'oceania', 'china', 'india', 'japan', 'russia', 'persia', 'ottoman', 'spain', 'france', 'england', 'egypt', 'rome'];
...
const loadTermsFromFile = async () => {
  try {
    if (!termsData) throw new Error('AP World History terms data not found.');
    const terms = Array.isArray(termsData) ? termsData : termsData.default || termsData.terms || [];
    if (!Array.isArray(terms) || terms.length === 0) throw new Error('AP World History terms data must be a non-empty array');
    ...
    console.error('Error loading AP World History terms:', err);
    return [];
  }
};
...
const [userAnswers, setUserAnswers] = useState({});
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-teal-100 flex items-center justify-center">
      <div className="text-xl font-medium text-gray-600">Loading AP World History Terms...</div>
    </div>
  );
      ...
      <div className="p-4">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center bg-green-50 p-4 rounded-lg">{currentTerm.term}</h2>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-4">
            ...
            <h1 className="text-4xl font-bold text-gray-800">AP World History Terms Quiz</h1>
          </div>
          <p className="text-gray-600 text-lg">Master your AP World History terms with interactive quizzes</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 mb-4">
            {availableUnits.map(unit => {
              ...
              const unitTermCount = worldTerms.filter(term => term.unit === unit).length;
              ...
              return (
                ...
                >
                  <div>Unit {unit}</div>
                  <div className={`text-xs ${isSelected ? 'text-green-200' : 'text-gray-500'}`}>
                    {unitTermCount} terms
                  </div>
                </button>
              );
        ...
        {filtered.length > 0 && (
          <div className="mt-6 bg-white rounded-lg shadow-lg p-6">
            ...
            <p className="text-gray-600 mb-4">Browse through the AP World History terms from your selected units:</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filtered.slice(previewIndex, previewIndex + 4).map((term, index) => (
                <div key={term.id || index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="mb-3">
                    <h3 className="font-bold text-lg text-green-600">{term.term}</h3>
                    {term.unit && (
