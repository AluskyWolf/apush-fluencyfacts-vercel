...
// Helper function to extract keywords from terms
const extractKeywords = (term) => {
  const keywords = [];
  if (term.who) keywords.push(...term.who.split(' '));
  if (term.what) keywords.push(...term.what.split(' '));
  if (term.where) keywords.push(...term.where.split(' '));
  if (term.when) keywords.push(...term.when.split(' '));
  if (term.why) keywords.push(...term.why.split(' '));
  return keywords.filter(keyword => keyword.length > 3); // Filter out short words
};
...
const QuizResults = ({ subject, mode, filtered, startTime, endTime, resetQuiz, startQuiz, getUnitSelectionDescription }) => {
  ...
  }, [subject, mode, duration, totalQuestions]);
...
const WorldTerms = () => {
  ...
  }, [selectedUnits, worldTerms, previewIndex, getFilteredTerms]);
  ...
  const submitAnswer = () => {
    ...
    // Track that question was answered (remove scoring parameter)
    fields.forEach(field => {
      trackQuestionAnswer('world', field, null, currentQuestion + 1);
    });
