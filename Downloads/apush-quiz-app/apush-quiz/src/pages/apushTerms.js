...
const calculateSimilarityScore = (userAnswer, correctAnswer, keywords = [], field = null) => {
  ...
  if (!meetsMinimumRequirements(userOriginal, field, correctAnswer)) {
    const hasRelevantWord = keywords.some(keyword => userNormalized.includes(keyword.toLowerCase()));
  ...
  if (field === 'who') {
    ...
    const userHasSpecificGroup = specificEuropeanGroups.some(group => userNormalized.includes(group));
...
const getScoreLevel = (fraction, userAnswer = '', field = null) => {
  ...
  if (fraction === 0 && userLength <= 3) {
    return { level: 'no answer', color: '#991B1B', bgColor: '#FEE2E2', icon: <XCircle className="w-5 h-5" />, message: 'Please provide a meaningful answer.' };
  }
  if (fraction >= 0.9) {
    return { level: 'excellent', color: '#10B981', bgColor: '#D1FAE5', icon: <CheckCircle className="w-5 h-5" />, message: 'Outstanding! You demonstrated comprehensive understanding.' };
  }
  if (fraction >= 0.75) {
    return { level: 'very good', color: '#059669', bgColor: '#D1FAE5', icon: <CheckCircle className="w-5 h-5" />, message: 'Very good! You captured the key concepts well.' };
  }
  if (fraction >= 0.6) {
    return { level: 'good', color: '#3B82F6', bgColor: '#DBEAFE', icon: <AlertCircle className="w-5 h-5" />, message: 'Good work! You got the main idea with some details.' };
  }
  if (fraction >= 0.4) {
    return { level: 'partial', color: '#F59E0B', bgColor: '#FEF3C7', icon: <AlertCircle className="w-5 h-5" />, message: 'Partially correct. You have some understanding but need more detail.' };
  }
  if (fraction >= 0.2) {
    return { level: 'needs improvement', color: '#DC2626', bgColor: '#FEE2E2', icon: <XCircle className="w-5 h-5" />, message: 'Your answer shows limited understanding. Please review this topic.' };
  }
  return { level: 'incorrect', color: '#991B1B', bgColor: '#FEE2E2', icon: <XCircle className="w-5 h-5" />, message: 'Incorrect. Please study this topic more thoroughly.' };
};
...
const APUSHTerms = () => {
  ...
  const submitAnswer = () => {
    ...
    fields.forEach(field => {
      ...
      scores[key] = score;
      trackQuestionAnswer('apush', field, score, currentQuestion + 1);
    });
